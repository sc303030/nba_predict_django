...
def crawling_position(position_dic):
    ...
    for names in list(position_dic.keys()):
        ...
        position_dic[names]['position'] = position_td
        # position_dic[names]['img_url'] = player_image
        key_words.clear()
        time.sleep(1)
    return position_dic
...
def write_to_dir(nba_injury_merge, position_dic):
    nba_injury_merge[['position', 'img_url']] = nba_injury_merge['name'].apply(lambda x: posi(x, position_dic))
